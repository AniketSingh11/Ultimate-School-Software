...
class Sattendance_m extends MY_Model {
	...
	function update_sub_attendance_classes_new($colname, $status, $schoolyearID, $classesID, $monthyear, $sectionID) {
		...
		$this->db->query("UPDATE $this->_table_name SET $colname='".$status."' WHERE schoolyearID = $schoolyearID AND classesID = $classesID AND monthyear = '".$monthyear."' AND sectionID = $sectionID AND ($colname ='L' OR $colname = 'A' OR $colname = 'P' OR $colname IS NULL)");
		return TRUE;
	}

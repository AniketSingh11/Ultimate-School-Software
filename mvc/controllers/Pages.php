        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $menuArray = array(
                    ...
                    'menu_pagesID' => $lastID,
                    'menu_orderID' => 10000,
                    'menu_label' => $this->input->post('title'),
                    'menu_status' => 1,
                );
                
                $this->fmenu_relation_m->insert_fmenu_relation($menuArray);

                frontendData::get_frontent_topbar_menu_delete();
                frontendData::get_frontent_topbar_menu();
                frontendData::get_page_delete();
                frontendData::get_page();
            ...
            if($this->data['page']) {
                if($_POST) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->fmenu_relation_m->update_fmenu_relation_by_array($menuArray, array('menu_pagesID' => $id));

                        if($status == 1) {
                            $this->fmenu_relation_m->update_fmenu_relation_by_array(array('menu_status' => 1), array('menu_pagesID' => $id));
                        } else {
                            $this->fmenu_relation_m->update_fmenu_relation_by_array(array('menu_status' => 0), array('menu_pagesID' => $id));
                        }
    ...
    public function delete() {
        ...
        if((int)$id) {
            ...
            if($this->data['pages']) {
                ...
                $this->fmenu_relation_m->delete_fmenu_relation_by_array(array('menu_pagesID' => $id));

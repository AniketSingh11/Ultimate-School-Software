...
class Invoice extends Admin_Controller {
						...
						if($this->data['invoice']->paidstatus != 2) {
							...
							if($_POST) {
								...
								if($this->form_validation->run() == FALSE) {
									...
									} elseif ($this->input->post('payment_method') == 'Voguepay') {
                                        $this->post_data = $this->input->post();
                                        $this->post_data['id'] = $this->uri->segment(3);
                                        $this->invoice_data = $this->invoice_m->get_invoice_with_studentrelation_by_invoiceID($this->post_data['id']);
                                        $this->voguepay($this->uri->segment(3), $this->input->post());
                                    } else {
    ...
    /* VoguePay*/
    public function voguepay($invoice, $request)
    {
        $api_config = array();
        $get_configs = $this->payment_settings_m->get_order_by_config();
        foreach ($get_configs as $key => $get_key) {
        ...
        $this->array['success_url'] = base_url('invoice/voguepay_success/'.$invoice);
        $this->array['notify_url'] = base_url('invoice/voguepay_notify/'.$invoice);
        $this->array['fail_url'] = base_url('invoice/voguepay_failed/'.$invoice);
        $this->array['action'] = $api_link;
        $this->array['total'] = $request['amount'];
        $this->array['merchant_ref'] = $api_config['voguepay_merchant_ref'];
        $this->array['developer_code'] = $api_config['voguepay_developer_code'];
        $this->array['memo'] =$this->array['invoice']->invoiceID;
    ...
    function voguepay_success() {
        ...
        if ($result->state=="success") {
            ...
            if(!count($dbTransactionID)) {
                $this->data['invoice'] = $this->invoice_m->get_single_invoice(array('invoiceID' => $invoiceID, 'deleted_at' => 1));
                $this->data['payment'] = $this->payment_m->get_payment_by_sum($invoiceID);
                $this->data['dueamount'] = ($this->data['invoice']->amount - ((($this->data['invoice']->amount/100) * $this->data['invoice']->discount) + $this->data['payment']->paymentamount));
                ...
                $payment_array = array(
                    ...
                    "invoiceID" => $invoiceID,
                    "studentID"	=> $this->data['invoice']->studentID,
                    "paymentamount" => floatval($result->details->total_paid_by_buyer),
                    "paymenttype" => 'Voguepay',
                    "paymentdate" => date('Y-m-d'),
                    "paymentday" => date('d'),
                    "paymentmonth" => date('m'),
                    "paymentyear" => date('Y'),
                ...
                $this->payment_m->insert_payment($payment_array);
                if(floatval($this->data['dueamount']) <= floatval($result->details->total_paid_by_buyer)) {
                    $this->invoice_m->update_invoice(array('paidstatus' => 2), $invoiceID);
                } else {
                    $this->invoice_m->update_invoice(array('paidstatus' => 1), $invoiceID);
                }
                $this->session->set_flashdata('success', 'Payment successful!');
    ...
    function verifyVoguepayPayment($transaction_id)
    {
    ...
    function getVoguepayPaymentDetails($transaction_id,$type="json")
    {
        $api_config = array();
        $get_configs = $this->payment_settings_m->get_order_by_config();
        foreach ($get_configs as $key => $get_key) {
	...
	function unique_paymentmethod() {
		if($this->input->post('payment_method') === '0') {
			...
			if($this->input->post('payment_method') == 'Cash' || $this->input->post('payment_method') == 'Cheque') {
				...
				return TRUE;
			} elseif ($this->input->post('payment_method') == 'Voguepay' && $api_config['voguepay_status'] == 1) {

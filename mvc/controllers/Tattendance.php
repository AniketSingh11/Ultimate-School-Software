...
class Tattendance extends Admin_Controller {
	...
	function singl_add() {
		...
		if((int)$id && (int)$day && $method && $status) {
			...
			if((int)$day && (int)$id) {
				$attendance_row = $this->tattendance_m->get_tattendance($id);
				if($attendance_row) {
					$this->tattendance_m->update_tattendance(array($aday => $status), $id);
	...
	function all_add() {
		...
		if((int)$day) {
			$this->tattendance_m->update_tattendance_new("a".abs($day), $status, $schoolyearID, $monthyear);
			echo $this->lang->line('menu_success');
		}
	}
